...
{
    public class DoubleClickSpecUserAction : ClickSpecUserAction
    {
        ...
        {
            Regex pattern1 = new Regex("DoubleClick\\s+'[\\w|\\s]+'\\s*(->\\s*'[\\w|\\s]+'\\s*)*");
            Match matcher1 = pattern1.Match(expression);

            Regex pattern4 = new Regex("DoubleClick(\\s*\\[(\\d+?);\\s*(\\d+?)\\]){0,1}");
            Match matcher4 = pattern4.Match(expression);
            string re = "";
            if (matcher1.Success)
            {
                string[] elements = Regex.Split(expression.Substring(DOUBLE_CLICK.Length).Trim(), "->");
                /**
                ...
                for (int fi = 0; fi < elements.Length; fi++)
                {
                    ...
                    if (re != null && !re.Equals(""))
                        re += getElementExp(otherNode) + ".DoubleClick();";
                }
                //return re;
            }
